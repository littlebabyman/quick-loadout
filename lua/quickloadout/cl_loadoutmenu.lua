...
local ptable = {}
local loadouts = {}
QuickLoadouts = {WepList = {}, RefList = {}, TempList = {}}
...
if file.Exists(dir .. gm .. "autosave.json", "DATA") then
    QuickLoadouts.TempList = util.JSONToTable(file.Read(dir .. gm .. "autosave.json", "DATA"))
end
...
local function GenerateLabel(frame, name, class, panel)
    ...
    if ispanel(panel) and ispanel(panel.image) then
        ...
        button.OnCursorEntered = function(self)
            ...
            if class and QuickLoadouts.RefList[class] then
                panel.image.Text = class
                local icon = QuickLoadouts.RefList[class].HudImage or QuickLoadouts.RefList[class].Image
                if icon then
                    wepimg = Material(icon, "mips")
                    local ratio = wepimg:Width() / wepimg:Height()
                    panel.image.ImageRatio = ratio - 1
                end
                local stats = QuickLoadouts.RefList[class].Stats
                if stats then
...
local function NetworkLoadout()
    ...
    lastgiven = CurTime()
    local weps = util.Compress(util.TableToJSON(QuickLoadouts.TempList))
...
function ShortenCategory(wep)
    local ref, match, cat, slot = QuickLoadouts.RefList[wep], "^[%w%d%p]+", showcat:GetBool(), showslot:GetBool()
        ...
        for class, wep in SortedPairs(QuickLoadouts.RefList) do
            if wep.Spawnable then
                ...
                if (cat) then
                    ...
                    wep.SubCategory = cat
                    if !QuickLoadouts.WepList[nicecat][cat] then
                        QuickLoadouts.WepList[nicecat][cat] = {}
                    end
                    table.ForceInsert(QuickLoadouts.WepList[nicecat][cat], {class = class, name = wep.PrintName})
                end
...
function QLOpenMenu()
    local tmp = {}
    table.CopyFromTo(QuickLoadouts.TempList, tmp)
    ...
    function CloseMenu(apply)
        ...
        file.Write(dir .. gm .. "autosave.json", util.TableToJSON(QuickLoadouts.TempList))
    ...
    GenerateWeaponTable()
    table.RemoveByValue(QuickLoadouts.TempList, "")
    ...
    mainmenu.image.Think = function(self)
        if !self.WepData.type then
            ...
            self.WepData.type = self.Text and QuickLoadouts.RefList[self.Text].Stats and ((!self.WepData.mag or !self.WepData.ammo or !self.WepData.dmgtotal) and 3 or 2) or 0
        end
    end
    ...
    importer.DoClickInternal = function(self)
        ...
        local g = 0
        table.Empty(QuickLoadouts.TempList)
        for k, v in ipairs(trash) do
            if holster and v:GetClass() == (QuickLoadouts.RefList[holster] and holster or "weaponholster") then g = 1 continue end
            QuickLoadouts.TempList[k-g] = v:GetClass()
        end
        CreateWeaponButtons()
        RefreshLoadout(closer)
    end
    ...
    modelpanel:SetModel(player_manager.TranslatePlayerModel(mdl:GetString()), mskin and mskin:GetInt() or 0, mbg and mbg:GetString() or "000000000")
    ...
    function mainmenu.theguy:SetWeapon( strModelName )
        ...
        self.Entity2:AddEffects(EF_PARENT_ANIMATES+EF_BONEMERGE_FASTCULL)
        if !IsValid(self.Entity) then return end
        self.Entity2:SetupBones()
        if self.Entity:LookupBone(self.Entity2:GetBoneName(0)) then
    ...
    function mainmenu.theguy:ResetParameters()
        ...
        self.Entity:SetSkin(mskin and mskin:GetInt() or 0)
        self.Entity:SetBodyGroups(mbg and mbg:GetString() or "000000000")
        function mainmenu.theguy.Entity:GetPlayerColor() return LocalPlayer():GetPlayerColor() end
    ...
    function QuickName(name)
        local ref = QuickLoadouts.RefList[name]
    ...
    function CreateWeaponButtons() -- it's a lot better now i think :)
        ...
        for i, v in ipairs(QuickLoadouts.TempList) do
        ...
        WepSelector(newwep, #QuickLoadouts.TempList + 1, nil)
    ...
    function PopulateCategory(parent, tbl, cont, cat, slot, noclear) -- good enough automated container refresh
        ...
        for key, v in SortedPairs(sublist) do
            ...
            local ref = QuickLoadouts.RefList[keyname]
            local haswep = (table.HasValue(QuickLoadouts.TempList, keyname) and !QuickLoadouts.TempList[slot])
            ...
            local cattext, weptext, eqnum = button.ShortCat, ref.SubCategory and (ref.Rating and ref.Rating .. " " or "") .. ref.SubCategory, table.HasValue(QuickLoadouts.TempList, keyname) and "#"..tostring(table.KeyFromValue(QuickLoadouts.TempList, keyname))
            if eqnum and QuickLoadouts.TempList[slot] and slot != tonumber(table.KeyFromValue(QuickLoadouts.TempList, keyname)) then
                eqnum = eqnum .. " ↔ " .. "#"..slot
            end
            button.Paint = function(self, x, y)
                local offset = math.min(x * 0.1, y * 0.5)
            ...
            button.DoClickInternal = function(self)
                if table.HasValue(QuickLoadouts.TempList, keyname) then
                    if !QuickLoadouts.TempList[slot] then self:SetToggle(true) return end
                    table.Merge(QuickLoadouts.TempList, {[table.KeyFromValue(QuickLoadouts.TempList, keyname)] = QuickLoadouts.TempList[slot]}, true)
                end
                table.Merge(QuickLoadouts.TempList, {[slot] = keyname}, true)
                cat:Clear()
                CreateWeaponButtons()
                RefreshLoadout(closer)
            end
    ...
    function LoadoutSelector(button, key)
        -- print(button, key)
        local wepcount = (loadouts[key] and #loadouts[key].weps or #QuickLoadouts.TempList) .. " weapons"
        button:SizeToContentsY(fontsize)
        button.PaintOver = function(self, x, y)
            local offset = math.min(x * 0.1, y * 0.5)
            surface.SetDrawColor(color_default)
        ...
        if button.ClassName == "DLabelEditable" then
            ...
            button.OnLabelTextChanged = function(self, text)
                ...
                table.Merge(loadouts, {[key] = {name = text, weps = QuickLoadouts.TempList}}, true)
                file.Write(dir .. gm .. "client_loadouts.json", util.TableToJSON(loadouts))
            ...
            button.DoClickInternal = function(self)
                ...
                QuickLoadouts.TempList = loadouts[key].weps
                RefreshLoadout()
                CloseMenu(true)
            end
            button.DoRightClick = function(self)
    ...
    function WepSelector(button, index, class)
        ...
        local ref = QuickLoadouts.RefList[class]
        ...
        button.DoClickInternal = function()
            ...
            category3:Hide()
            category3:Clear()
            PopulateCategory(button, QuickLoadouts.WepList, rscroller, category1, index)
        ...
        button.DoRightClick = function(self)
            self:SetToggle(true)
            self:Toggle()
            if index > #QuickLoadouts.TempList then return end
            table.remove(QuickLoadouts.TempList, index)
            CreateWeaponButtons()
            RefreshLoadout(closer)
        end
        button:InvalidateLayout(true)
